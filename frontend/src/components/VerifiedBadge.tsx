...
export default function VerifiedBadge({ className = "w-5 h-5" }: VerifiedBadgeProps) {
  return (
    <div className={`relative inline-flex items-center justify-center ${className}`}>
      ...
      <svg 
        viewBox="0 0 24 24" 
        className="absolute inset-0 w-full h-full animate-spin text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.6)]"
      ...
      {/* Static checkmark (Dark so it contrasts with the white background) */}
      <svg 
        viewBox="0 0 24 24" 
        className="relative z-10 w-3/5 h-3/5 text-[#080b1a]" 
        fill="none" 
        stroke="currentColor" 
        strokeWidth="3.5" 
        strokeLinecap="round" 
